...
class ERC_model(nn.Module):
    def __init__(self, model_type, clsNum, freeze, initial):
        ...
        self.context_model = AutoModel.from_pretrained(model_path)

        # Scartch or pre-trained
#         if initial == 'scratch':
#             config = AutoConfig.from_pretrained(phobert)
#             self.speaker_model = AutoModel.from_config(config)
        if initial == 'scratch':
        ...
        # Nếu freeze -> không cập nhật ở PM
        if not freeze:
    ...
    def forward(self, batch_input_tokens, batch_speaker_tokens):
        ...
        batch_context_output = self.context_model(batch_input_tokens).last_hidden_state[:,0,:] # (batch, 768)
        ...
        for speaker_tokens in batch_speaker_tokens:
            if speaker_tokens.shape[0] == 0:
                ...
                speaker_output = self.speaker_model(speaker_tokens.cuda()).last_hidden_state[:,0,:] # (speaker_utt_num, 768)
